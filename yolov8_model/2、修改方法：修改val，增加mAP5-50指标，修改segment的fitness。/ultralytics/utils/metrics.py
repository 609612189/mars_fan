...
class Metric(SimpleClass):
    ...
    def ap(self):
        ...
        return self.all_ap[:,9:].mean(1) if len(self.all_ap) else []
    ...
    def map75(self):
        ...
        return self.all_ap[:, 14].mean() if len(self.all_ap) else 0.0
...
class SegmentMetrics(SimpleClass):
    ...
    def keys(self):
        return [
            'metrics/precision(B)', 'metrics/recall(B)', 'metrics/mAP50(B)', 'metrics/mAP5-50(B)', 'metrics/mAP50-95(B)',
            'metrics/precision(M)', 'metrics/recall(M)', 'metrics/mAP50(M)', 'metrics/mAP5-50(M)', 'metrics/mAP50-95(M)']
    ...
    def mean_results(self):
        """Return the mean metrics for bounding box and segmentation results."""
        return [self.box.mp, self.box.mr, self.box.map50, self.box.map5to50, self.box.map] + [self.seg.mp, self.seg.mr, self.seg.map50, self.box.map5to50, self.seg.map]
    ...
    @property
    def fitness(self):
        return (self.box.map50 * 0.9 + self.box.map * 0.1)
